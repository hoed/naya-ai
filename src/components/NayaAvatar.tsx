...
const NayaAvatar = ({ state }: NayaAvatarProps) => {
  const getStateClass = () => {
    switch (state) {
      case "listening":
        return "naya-avatar-listening border-accent shadow-[0_0_50px_rgba(234,88,12,0.2)]";
      case "talking":
        return "naya-avatar-talking border-primary shadow-[0_0_60px_rgba(20,184,166,0.2)]";
      default:
        return "naya-avatar-idle border-white/10 shadow-lg";
    }
  };
  ...
  return (
    <div className="relative flex flex-col items-center">
      ...
      <div
        className={`absolute -inset-8 rounded-full blur-3xl transition-all duration-1000 ${state === "listening" ? "opacity-40 bg-accent" :
      ...
      >
        <img
          ...
          alt="AISA Avatar"
          className={`h-full w-full object-cover transition-all duration-700 ${state === "talking" ? "scale-110 rotate-1" :
      ...
      <div className="mt-8 flex flex-col items-center gap-2">
        <div className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/5 backdrop-blur-md border border-white/10 shadow-sm">
          <div
            className={`h-2 w-2 rounded-full transition-all duration-500 ${state === "listening"
